...
class ButtonRoleCog(commands.Cog):
    ...
    async def post(self, ctx: discord.ApplicationContext):
        ...
        # make a secind view
        view_two = discord.ui.View(timeout=None)
        for role_id in role_ids:
            # Get the role from the guild by ID.
            role = ctx.guild.get_role(role_id)
            view.add_item(RoleButton(role))
        # add embed to the view
        # print cyrrent path
        print(f"Current path: {os.getcwd()}")


        # get role picture
        filename = f"reactroles_pings.png"
        await ctx.send(file=discord.File(open(filename, "rb")))  # send the file to discord)
        # send the message
        await ctx.send("React to this message to get the role you want!", view=view)
