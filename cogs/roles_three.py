...
role_ids = [1070181260246405230]
guild_ids = [1066139205060804679, 1030138986758406286]


class RoleButtonThree(discord.ui.Button):
...
class ButtonRoleCogThree(commands.Cog):
    """
    ...
    async def postthree(self, ctx: discord.ApplicationContext):
        """Slash command to post a new view with a button for each role."""

        # timeout is None because we want this view to be persistent.
        view = discord.ui.View(timeout=None)
        filename = f"reactroles_plush.png"
        ...
        for role_id in role_ids:
            ...
            view.add_item(RoleButtonThree(role))
...
def setup(bot):
    bot.add_cog(ButtonRoleCogThree(bot))
